...
import MetricChart from '@/components/dashboard/MetricChart';
import RoomCard from '@/components/dashboard/RoomCard';
import { Thermometer, Droplets, ArrowUp, ArrowDown, Gauge } from 'lucide-react';
import { getRoomsData, getCurrentReadings, getOutdoorReadings, RoomData } from '@/services/airQualityService';
import { log } from 'console';
...
const Sense = () => {
  ...
  const [temperatureData, setTemperatureData] = useState<{ timestamp: string; value: number }[]>([]);
  const [humidityData, setHumidityData] = useState<{ timestamp: string; value: number }[]>([]);
  const [pressureData, setPressureData] = useState<{ timestamp: string; value: number }[]>([]);
  const [currentReadings, setCurrentReadings] = useState(getCurrentReadings());
  const [outdoorReadings, setOutdoorReadings] = useState(getOutdoorReadings());
  const [rooms, setRooms] = useState<RoomData[]>(getRoomsData());
  ...
  useEffect(() => {
    if (sensorData.length > 0) {
      ...
      const pressureSeries = sensorData.map(item => ({
        timestamp: item.timestamp,
        value: item.pressure,
      }));
      ...
      setTemperatureData(tempSeries);
      setHumidityData(humSeries);
      setPressureData(pressureSeries);
    }
  }, [timeRange, sensorData]);
  ...
  const getPressureStatus = (pressure: number) => {
    if (pressure < 980 || pressure > 1050) return 'bad';
    if (pressure < 990 || pressure > 1030) return 'medium';
    return 'good';
  };
    ...
    <div className="container mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <MetricCard
          ...
          status={getPressureStatus(currentReadings.pressure)}
          subtitle="Indoor Average"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <Card className="border-none shadow-md bg-airvibe-cardbg">
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* <ComparisonCard
                metric="Pressure"
                indoor={currentReadings.pressure}
                outdoor={outdoorReadings.pressure}
                unit="hPa"
                icon={<Gauge className="h-5 w-5 text-white" />}
              /> */}
            </div>
        ...
        <Tabs defaultValue="temperature">
          <TabsList className="grid grid-cols-3 bg-airvibe-graybg">
            <TabsTrigger value="temperature">Temperature</TabsTrigger>
            <TabsTrigger value="humidity">Humidity</TabsTrigger>
